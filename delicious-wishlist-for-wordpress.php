...
function wdw_options_validate( $input ) {
	...
	$input[ 'wdw_delicious_howmany' ] = absint( intval( $input[ 'wdw_delicious_howmany' ] ) );
	if ( $input[ 'wdw_delicious_howmany' ] > 100 ) {
		$input[ 'wdw_delicious_howmany' ] = 100;
	}

	// Cache value must be integer and not minor than 3600
	$input[ 'wdw_delicious_cache' ] = absint( intval( $input[ 'wdw_delicious_cache' ] ) );
	...
	$input['wdw_delicious_alt_feed_ht']  = esc_url( $input['wdw_delicious_alt_feed_ht'] );
	$input['wdw_delicious_alt_feed_mt']  = esc_url( $input['wdw_delicious_alt_feed_mt'] );
	$input['wdw_delicious_alt_feed_lt']  = esc_url( $input['wdw_delicious_alt_feed_lt'] );
...
function wdw_fetch_bookmarks( $args ) {
	...
	// ... else execute...
		...
		if ( 'space' == $wdw_tag_sep ) {
			$wdw_tag_sep = ' ';
		} elseif ( 'comma' == $wdw_tag_sep ) {
			$wdw_tag_sep = ',';
		} elseif ( 'comma-space' == $wdw_tag_sep ) {
			$wdw_tag_sep = ', ';
		}
				...
				foreach ( $wdw_items as $wdw_item ) :
					...
					$wdw_wishlist .= '<li class="' . $list_class . '-' . $wdw_icons . '">';
						...
						if ( ( in_the_loop() && $wdw_date ) || ( ! in_the_loop() && $widget_date ) ) {
							...
							$unix_date  = strtotime( $wdw_item->get_date() );
							// This is the date in localized format
							$localized_date = date_i18n( get_option( 'date_format' ), $unix_date );
							...
							$wdw_wishlist .= '<p class="wishlist-timestamp">
								...
								title="' . $a_title . '">' . $localized_date . '</a></p>';
						}
